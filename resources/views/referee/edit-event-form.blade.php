...
@section('main')
    <div class="card shadow-sm p-3 bg-body rounded">
        <h2>Edit Event Wasit</h2>
        <p>Edit data event Anda sebagai wasit basket pada form ini.</p>
        @include('partials.alert')
        <form action="{{ route('referee.editEvent', $event->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <div class="row">
                    <div class="col">
                        ...
                        <input type="date" id="tanggal-mulai" name="start_date" class="form-control mt-1"
                            placeholder="Tanggal Mulai" value="{{ $event->start_date }}" required>
                    </div>
                    <div class="col">
                        ...
                        <input type="date" id="tanggal-berakhir" name="end_date" class="form-control mt-1"
                            placeholder="Tanggal Berakhir" value="{{ $event->end_date }}" required>
                    </div>
                </div>
                ...
                <select class="form-control mt-3" id="kota" name="level_id" required>
                    ...
                    @foreach ($levels as $level)
                        <option value="{{ $level->id }}" {{ $event->level_id == $level->id ? 'selected' : '' }}>
                            {{ $level->name }}</option>
                    @endforeach
                </select>
                ...
                <input type="text" id="posisi-jabatan" name="position" class="form-control mt-1"
                    placeholder="Posisi/Jabatan Wasit" value="{{ $event->position }}" required>
                ...
                <div class="d-flex flex-row">
                    <div class="d-flex flex-column flex-grow-1">
                        ...
                        <input type="text" id="nama-tim" name="matches[0][]" class="form-control mt-1"
                            placeholder="Nama Tim A" value="{{ $game->team_a }}">
                    </div>
                    ...
                    <div class="d-flex flex-column flex-grow-1">
                        ...
                        <input type="text" id="nama-tim" name="matches[1][]" class="form-control mt-1"
                            placeholder="Nama Tim B" value="{{ $game->team_b }}">
                    </div>
                </div>
            @empty
                <div class="d-flex flex-row">
